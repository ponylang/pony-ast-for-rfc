...
class \nodoc\ iso _CompileSimple is UnitTest
  ...
  fun apply(h: TestHelper) =>
    let source_dir = Path.join(Path.dir(__loc.file()), "_test_fixtures/simple")
    let pony_path =
      try
...
class \nodoc\ iso _PositionIndexFind is UnitTest
  ...
  fun apply(h: TestHelper) =>
    let source_dir = Path.join(Path.dir(__loc.file()), "_test_fixtures/constructs")
    let pony_path =
      try
        PonyPath(h.env) as String
      else
...
class \nodoc\ iso _DefinitionTest is UnitTest
...
class \nodoc\ iso _EqHashTest is UnitTest
...
class \nodoc\ iso _CompileErrorTest is UnitTest
  ...
  fun apply(h: TestHelper)? =>
    ...
    for expected_errs in this.expected_errors.values() do
      let source_dir = Path.join(Path.dir(__loc.file()), "_test_fixtures/" + expected_errs.folder)
